...
def update_product_admin(product_id):
    ...
    if "price" in data:
        try:
            p.price = Decimal(str(data.get("price") or "0"))
        except Exception:
            return jsonify({"ok": False, "error": "price inválido"}), 400
...
# =========================================================
# REPORTES PRO (HTML + API) - ADAPTADO A TU MODELO REAL
# =========================================================
...
def admin_api_reportes():
    ...
    """
    try:
        from app.models import Order, OrderItem, Payment, User, CashRegister, OrderStatus
        ...
        end_dt = datetime(d_to.year, d_to.month, d_to.day) + timedelta(days=1)  # exclusivo

        # Base query: orders closed en rango
        ...
        # Map cajas y usuarios (para nombres)
        ...
        for o in orders:
            ...
            # método pago (si hay múltiples pagos, sumamos todos)
            if o.payments:
                for p in o.payments:
            ...
            # Para tabla: método “principal” (primero) o “Mixto”
            pm_label = "Sin método"
        ...
        sales_by_payment = []
        # orden fijo: cash/transfer/otros
...
def admin_reportes_export_xlsx():
    # Lo dejamos como stub por ahora
